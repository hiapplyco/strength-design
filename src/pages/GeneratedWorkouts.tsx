    ...
    const fetchWorkouts = async () => {
      try {
        const {
          data,
          error
        } = await supabase.from('generated_workouts').select('*').order('generated_at', {
          ascending: false
        });
    ...
    backgroundImage: 'url("/lovable-uploads/47062b35-74bb-47f1-aaa1-a642db4673ce.png")'
      ...
      <div className="min-h-screen bg-black/75 backdrop-blur-sm">
        <div className="container mx-auto px-4 py-16">
          ...
          {isLoading ? <p className="text-white text-center">Loading your workouts...</p> : <div className="grid gap-4">
              {workouts.map(workout => <Card key={workout.id} className="bg-white/10 hover:bg-white/20 transition-colors cursor-pointer border-2 border-primary/20" onClick={() => handleWorkoutClick(workout)}>
                    ...
                    <ScrollArea className="h-[100px] w-full border border-white/20 p-4 px-[13px] py-[12px] rounded-lg">
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-gray-400">
                          {workout.generated_at ? formatDistanceToNow(new Date(workout.generated_at), {
                      addSuffix: true
                    }) : "Recently generated"}
                        </p>
                      </div>
