...
export const CalendarExportDialog = ({ open, onOpenChange, workouts }: CalendarExportDialogProps) => {
  ...
  const saveWorkoutSessions = async (startDate: Date, events: ReturnType<typeof prepareWorkoutEvents>) => {
    ...
    try {
      // First, create a generated workout entry
      const workoutData = {
      ...
      const workoutId = await workoutQueries.createWorkout(user.uid, workoutData);

      // Create workout sessions for each day
      const sessionPromises = events.map(async (event) => {
        ...
        const sessionData = {
          ...
          workout_data: event.workoutData
        };

        return workoutQueries.createWorkoutSession(user.uid, sessionData);
      });
