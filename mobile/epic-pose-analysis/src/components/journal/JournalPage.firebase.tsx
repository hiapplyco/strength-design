...
import { useJournalEntries } from '@/hooks/useJournalEntries.firebase';
import { useWorkoutSessions } from '@/hooks/useWorkoutSessions.firebase';
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Tabs defaultValue="calendar" className="space-y-6">
                  ...
                  {recentEntries.map((entry) => (
                    <div key={entry.id} className="p-4 border rounded-lg space-y-2">
                      <div className="flex justify-between items-start">
                        <h4 className="font-medium">{entry.title || 'Untitled Entry'}</h4>
        ...
        <TabsContent value="insights" className="space-y-6">
          <InsightsTab entries={entries} sessions={sessions} />
        </TabsContent>
      </Tabs>
    </div>
  );
