...
The Vision MCP provides tools for interacting with camera hardware and performing advanced vision-based AI tasks.

### Available Tools

-   **`list_cameras`**:
    -   **Description**: Probes camera indexes from 0 to `max_index`-1 to discover and list all available camera devices.
    -   **Use Case**: Useful for allowing users to select a specific camera when multiple are connected.

-   **`vision_start`**:
    -   **Description**: Opens and initializes a camera with specified settings.
    -   **Parameters**: `camera_index`, `width`, `height`, `fps`, `backend` (auto, avfoundation, msmf, dshow, v4l2).
    -   **Use Case**: Prepares the camera for capturing frames.

-   **`vision_status`**:
    -   **Description**: Reports whether a camera is currently open and lists its active properties.
    -   **Use Case**: Checking the state of the camera before attempting a capture.

-   **`vision_capture`**:
    -   **Description**: Captures a single frame from the open camera. The frame is saved to a specified directory, and no base64 data is returned directly.
    -   **Use Case**: Taking a single photo or snapshot.

-   **`vision_burst`**:
    -   **Description**: Captures a sequence of frames at a specified interval.
    -   **Use Case**: Creating time-lapses, GIFs, or collecting frames for sequence analysis (e.g., ASL interpretation).

-   **`vision_stop`**:
    -   **Description**: Releases the camera hardware, making it available for other applications.
    -   **Use Case**: Should be called after all capture operations are complete to free up resources.

-   **`banana_generate`**:
    -   **Description**: Generates one or more images from a text prompt, optionally using an input image as a guide (image-to-image generation).
    -   **Use Case**: Creating stylized AI-generated images from a webcam capture.

-   **`veo_generate_video`**:
    -   **Description**: Generates a video sequence based on a text prompt, optionally conditioned by an initial input image.
    -   **Use Case**: Animating a still image (either a raw capture or a `banana_generate` output) into a short video clip.
